...
The RPM spec file provided here is for Relay-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://relay.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Relay git repository](https://github.com/relay/relay) but are not part of
...
Currently only `Source10` is used, for the example `relay.conf` file.
...
in the relay git repository. At present only an SVG version of the Relay
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the relay
...
needed when 0.12.1 is released, a proper fix is already in the Relay git
master branch.
...
idea to build Relay Core against a static version of that library compiled
...
The `relay.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `relay-server` package. At this point, most RPM based
...
The `relay-server` package should have SELinux support. How to properly do
...
Relay-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/relay/). He has been
packaging Relay for Fedora at least since 2012.
...
4. I renamed the `relay` package that contains the Qt GUI to `relay-core` as
that appears to be how the general population refers to it, in contrast to
`relay-xt` or `relay-classic`. I wanted to make sure the general population
