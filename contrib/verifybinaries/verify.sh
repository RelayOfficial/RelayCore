...
###   This script attempts to download the signature file SHA256SUMS.asc from relay.org
...
WORKINGDIR="/tmp/relay"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://relay.org/bin/"
VERSIONPREFIX="relay-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'relay-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Relay Core binary release signing key installed?"
   fi
