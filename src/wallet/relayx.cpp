...
UniValue RelayX::CreateAnonTransaction(string address, CAmount nValue, int nTransactions) {
  ...
  if (encryptionSuccess) {
    UniValue addresses = find_value(serverData, "rly_addresses");
    UniValue addrArray = addresses.get_array();

    UniValue relayXData;
    relayXData.setObject();
    relayXData.pushKV("anondestination", encryptedAddress);
    relayXData.pushKV("public_key", pubKey);
    relayXData.pushKV("anonaddress", addrArray);
    relayXData.pushKV("anonfee", find_value(serverData, "transaction_fee"));
    relayXData.pushKV("min_amount", find_value(serverData, "min_amount"));
    return relayXData;
  } else {
    throw runtime_error("Unable to send RelayX transaction, please try again.");
  }

}

vector<anonServer> RelayX::GetAnonServers() {

  vector<anonServer> returnServers;

  if (vAddedAnonServers.size() < 1 && mapMultiArgs["-addanonserver"].size() < 1) {
      throw runtime_error("You must have at least one RelayX server added to your conf file or by rpc command.");
  ...
  if (returnServers.size() < 1) {
      throw runtime_error("Please, add some valid RELAYX server to send private payments.");
      return returnServers;
  }
...
UniValue RelayX::FindAnonServer(std::vector<anonServer> anonServers, CAmount nValue, int nTransactions) {
  if (anonServers.size() < 1) {
      throw runtime_error("None of your configured RELAYX nodes are available right now.");
  }
...
UniValue RelayX::ParseJSONResponse(string readBuffer) {
  try {
      UniValue valRequest;
...
string RelayX::EncryptAddress(string address, string pubKeyStr, int nTransactions, int nPiece, long nId) {

  address = "{\"n\":\""+address+"\",\"t\":"+std::to_string(GetArg("anon_out_delay",RELAYX_DEFAULT_OUT_DELAY))+",\"p\":"+std::to_string(nPiece)+",\"o\":"+std::to_string(nTransactions)+",\"u\":"+std::to_string(nId)+"}";
...
int RelayX::PublicEncrypt(unsigned char* data, int data_len, unsigned char* key, unsigned char* encrypted)
{
...
RSA * RelayX::CreateRSA(unsigned char * key,int isPublic)
{
    RSA *rsa= NULL;
    BIO *keybio ;
...
bool RelayX::TestEncryption(string encrypted, UniValue serverData) {
...
UniValue RelayX::GetServerInfo(std::string server) {
  curl_global_init(CURL_GLOBAL_ALL);
  curl = curl_easy_init();
  string readBuffer;
  ...
  if (curl) {
    ...
    if (res != CURLE_OK) {
      ...
      if (!data_obj.isObject()) {
        ...
        UniValue relayXData;
        relayXData.setObject();
        relayXData.pushKV("min_amount", find_value(data, "min_amount"));
        relayXData.pushKV("max_amount", find_value(data, "max_amount"));
        relayXData.pushKV("transaction_fee", find_value(data, "transaction_fee"));
        curl_easy_cleanup(curl);
        curl_global_cleanup();
        return relayXData;
      }
