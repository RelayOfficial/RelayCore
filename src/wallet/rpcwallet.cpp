...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new Relay address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) DEPRECATED. The account name for the address to be linked to. If not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"relayaddress\"    (string) The new relay address\n"
            "\nExamples:\n"
    ...
    return CRelayAddress(keyID).ToString();
}


CRelayAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    CPubKey pubKey;
    ...
    return CRelayAddress(pubKey.GetID());
}
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Relay address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"relayaddress\"   (string) The account relay address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Relay address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"relayaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"relayaddress\"  (string, required) The relay address to be associated with an account.\n"
    ...
    CRelayAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Relay address");
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"relayaddress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"relayaddress\"  (string, required) The relay address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"relayaddress\"  (string) a relay address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    BOOST_FOREACH(const PAIRTYPE(CRelayAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        const CRelayAddress& address = item.first;
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw runtime_error(
            "sendtoaddress \"relayaddress\" amount ( \"comment\" \"comment-to\" \"anon-destination\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"relayaddress\"  (string, required) The relay address to send to.\n"
            ...
            "5. \"anon-destination\"  (string, optional) Encrypted destination address if you're sending a RELAYX transaction \n"
            "6. subtractfeefromamount  (boolean, optional, default=false) The fee will be deducted from the amount being sent.\n"
            "                             The recipient will receive less relays than you enter in the amount field.\n"
            "\nResult:\n"
    ...
    CRelayAddress address(address_str);
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Relay address");
...
UniValue createproposal(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 4)
        throw runtime_error(
            ...
            "\nCreates a proposal for the community fund. Min fee of " + std::to_string((float)Params().GetConsensus().nProposalMinimalFee/COIN) + "RELAY is required.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"relayaddress\"     (string, required) The relay address where coins would be sent if proposal is approved.\n"
    ...
    CRelayAddress address("NQFqqMUD55ZV3PJEJZtaKCsQmjLT6JkjvJ"); // Dummy address
    ...
    CRelayAddress destaddress(Address);
...
UniValue createpaymentrequest(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nCreates a proposal to withdraw funds from the community fund. Fee: 0.0001 RELAY\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
    ...
    CRelayAddress address(proposal.Address);

    if(!address.IsValid())
        throw JSONRPCError(RPC_TYPE_ERROR, "Address of the proposal is not a valid Relay address.");
    ...
    std::string Secret = "I kindly ask to withdraw " + std::to_string(nReqAmount) + "RELAY from the proposal " + proposal.hash.ToString() + ". Payment request id: " + id;
...
UniValue donatefund(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "                             The fund will receive less relays than you enter in the amount field.\n"
            "\nResult:\n"
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n"
...
UniValue anonsend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "anonsend \"relayaddress\" amount ( \"comment\" \"comment-to\" )\n"
    ...
    int nEntropy = GetArg("anon_entropy",RELAYX_DEFAULT_ENTROPY);
    ...
    UniValue relayXData = relayX.CreateAnonTransaction(params[0].get_str(), nAmount / (nTransactions * 2), nTransactions);
    std::vector<UniValue> serverRelayAddresses(find_value(relayXData, "anonaddress").getValues());

    if(serverRelayAddresses.size() != nTransactions)
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "RELAYX server returned a different number of addresses.");

    for(unsigned int i = 0; i < serverRelayAddresses.size(); i++)
    {
        CRelayAddress serverRelayAddress(serverRelayAddresses[i].get_str());
        if (!serverRelayAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Relay address provided by RELAYX server");
    }
    ...
    CAmount nMinAmount = find_value(relayXData, "min_amount").get_int() * COIN;
    UniValue pubKey = find_value(relayXData, "public_key");
    ...
    for(unsigned int i = 0; i < serverRelayAddresses.size(); i++)
    {
        ...
        if(i == serverRelayAddresses.size() - 1 || (nAmountNotProcessed - nAmountToSubstract) < (nMinAmount + 0.001))
        {
            nAmountRound = nAmountNotProcessed;
            i = serverRelayAddresses.size();
        }
        else
        {
        ...
        string encryptedAddress = relayX.EncryptAddress(params[0].get_str(), pubKey.get_str(), serverRelayAddresses.size(), i+(i==serverRelayAddresses.size()?0:1), nId);
        wtx.strDZeel = encryptedAddress;
        SendMoney(serverRelayAddress.Get(), nAmountRound, fSubtractFeeFromAmount, wtx, encryptedAddress);
    }
...
UniValue getanondestination(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getanondestination \"relayaddress\"\n"
            ...
            "\"anonaddress\"  (string) The relay coin address to send the anon transaction to.\n"
            "\nExamples:\n"
    ...
    UniValue relayXData = relayX.CreateAnonTransaction(params[0].get_str());

    CRelayAddress serverRelayAddress(find_value(relayXData, "anonaddress").get_str());
    if (!serverRelayAddress.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Relay address provided by RELAYX server");
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"relayaddress\",     (string) The relay address\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        BOOST_FOREACH(CTxDestination address, grouping)
        {
            ...
            addressInfo.push_back(CRelayAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                if (pwalletMain->mapAddressBook.find(CRelayAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CRelayAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"relayaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"relayaddress\"  (string, required) The relay address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"relayaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given relayaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"relayaddress\"  (string, required) The relay address for transactions.\n"
    ...
    // Relay address
    CRelayAddress address = CRelayAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Relay address");
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"torelayaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a relay address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"torelayaddress\"  (string, required) The relay address to send funds to.\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CRelayAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Relay address");
    CAmount nAmount = AmountFromValue(params[2]);
    if (nAmount <= 0)
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The relay address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less relays than you enter in their corresponding amount field.\n"
    ...
    {
        CRelayAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Relay address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a Relay address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of relay addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) relay address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"relayaddress\"  (string) A relay address associated with the keys.\n"
    ...
    return CRelayAddress(innerID).ToString();
}
...
UniValue addwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    return CRelayAddress(w.result).ToString();
}
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    map<CRelayAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CRelayAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CRelayAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"relayaddress\",    (string) The relay address of the transaction. Not present for \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"relayaddress\",    (string) The relay address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"relayaddress\",   (string) The relay address involved in the transaction\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending relays\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending relay\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"relayaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; Relay server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending relays.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of relay addresses to filter\n"
            "    [\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the relay address\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CRelayAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Relay address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if (fValidAddress) {
            entry.push_back(Pair("address", CRelayAddress(address).ToString()));
...
UniValue fundrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"     (string, optional, default pool address) The relay address to receive the change\n"
    ...
    if (params.size() > 1) {
      if (params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            CRelayAddress address(options["changeAddress"].get_str());

            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_PARAMETER, "changeAddress must be a valid relay address");
...
UniValue resolveopenalias(const UniValue& params, bool fHelp)
{
  ...
  if ((fHelp || params.size() != 1))
      throw runtime_error(
          ...
          "\nResolves the give OpenAlias address to a Relay address.\n"
          "\nArguments:\n"
          "1. \"address\"    (string) The OpenAlias address.\n"
          "\nExamples:\n"
          "\nGet information about an OpenAlias address\n"
          + HelpExampleCli("resolveopenalias", "\"donate@relay.org\"")
      );
