...
void ImportAddress(const CRelayAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CRelayAddress(CScriptID(script)), strLabel);
    } else {
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CRelayAddress(pubKey.GetID()), strLabel);
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    return CRelaySecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CRelayAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CRelaySecret(key).ToString(), strTime);
