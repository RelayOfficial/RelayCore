...
{
    return QCoreApplication::translate("relay-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. relay_de.qm (shortcut "de" needs to be defined in relay.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. relya_de_DE.qm (shortcut "de_DE" needs to be defined in relay.qrc)
...
{
    QMessageBox::critical(0, "Runaway exception", NavCoinGUI::tr("A fatal error occurred. Relay Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(relay);
    Q_INIT_RESOURCE(relay_locale);
    ...
    /// 6. Determine availability of data directory and parse relay.conf
