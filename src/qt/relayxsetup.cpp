...
#include "relayxitem.h"
#include "ui_relayxsetup.h"
#include "net.h"
#include "skinize.h"
#include "util.h"
...
void navtechsetup::removeNavtechServer()
{
    ...
    QMessageBox::StandardButton btnRetVal = QMessageBox::question(this, tr("Remove RelayX server"),
        tr("You are about to remove the following RelayX server: ") + "<br><br>" + serverToRemove + "<br><br>" + tr("Are you sure?"),
...
void navtechsetup::getinfoNavtechServer()
{
    ...
    try {
        RelayX *navtech = new RelayX();
        UniValue data = navtech->GetServerInfo(serverToCheck.toStdString());

        QMessageBox::information(this, windowTitle(),
            tr("RelayX server") + "<br><br>" +  tr("Address: ") + serverToCheck + "<br>" + tr("Min amount: ") + QString::number(data["min_amount"].get_real()) + " <br>" + tr("Max amount: ") + QString::number(data["max_amount"].get_real()) + "<br>" + tr("Tx fee: ") + QString::number(data["transaction_fee"].get_real()) + "%",
            QMessageBox::Ok, QMessageBox::Ok);
    }
    catch(const std::runtime_error &e)
    {
        QMessageBox::warning(this, tr("RelayX server"),
