...
bool WalletModel::validateAddress(const QString &address)
{
  ...
  CRelayAddress addressParsed(address_str);
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered navcoin address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CRelayAddress(rcp.isanon ? rcp.destaddress.toStdString() : rcp.address.toStdString()).Get());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
    {
        ...
        if (!rcp.paymentRequest.IsInitialized())
        {
            ...
            CTxDestination dest = CRelayAddress(strAddress).Get();
...
{
    QString strAddress = QString::fromStdString(CRelayAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CRelayAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CRelayAddress(sAddress).Get();
