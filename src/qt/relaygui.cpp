...
#include "config/relay-config.h"
#endif

#include "main.h"
#include "relaygui.h"
#include "relayunits.h"
...
void NavCoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Relay address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and relay: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Relay addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Relay addresses"));
    ...
    openAction->setStatusTip(tr("Open a relay: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Relay command-line options").arg(tr(PACKAGE_NAME)));
...
void NavCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        topMenuLogo->setFixedSize(150,30);
        topMenuLogo->setObjectName("navLogo");
        topMenuLogo->move(0,0);
        topMenuLogo->setStyleSheet(
           "#navLogo { border-image: url(:/icons/logo_ns)  0 0 0 0 stretch stretch; border: 0px; }");

        topMenu1 = new QPushButton("Home");
        topMenu1->setFixedSize(150,94);
        topMenu1->setObjectName("topMenu1");
        connect(topMenu1, SIGNAL(clicked()), this, SLOT(gotoOverviewPage()));
        //topMenu1->move(186,0);
        topMenu1->setStyleSheet(
           "#topMenu1 { color:white; border: 0px; }"
           "#topMenu1:hover { color:#7A7D7D; border: 0px; }");

        topMenu2 = new QPushButton("Send");
        topMenu2->setFixedSize(150,94);
        topMenu2->setObjectName("topMenu2");
        connect(topMenu2, SIGNAL(clicked()), this, SLOT(gotoSendCoinsPage()));
        //topMenu2->move(325,0);
        topMenu2->setStyleSheet(
                    "#topMenu2 { color:white; border: 0px; }"
                    "#topMenu2:hover { color:#7A7D7D; border: 0px; }");

        topMenu3 = new QPushButton("Receive");
        topMenu3->setFixedSize(150,94);
        topMenu3->setObjectName("topMenu3");
        connect(topMenu3, SIGNAL(clicked()), this, SLOT(gotoRequestPaymentPage()));
        //topMenu3->move(469,0);
        topMenu3->setStyleSheet(
                    "#topMenu3 { color:white; border: 0px; }"
                    "#topMenu3:hover { color:#7A7D7D; border: 0px; }");

        topMenu4 = new QPushButton("Transactions");
        topMenu4->setFixedSize(150,94);
        topMenu4->setObjectName("topMenu4");
        connect(topMenu4, SIGNAL(clicked()), this, SLOT(gotoHistoryPage()));
        //topMenu4->move(625,0);
        topMenu4->setStyleSheet(
                    "#topMenu4 { color:white; border: 0px; }"
                    "#topMenu4:hover { color:#7A7D7D; border: 0px; }");
        walletFrame->topMenuLayout->addWidget(topMenuLogo);
        walletFrame->topMenuLayout->addWidget(topMenu1);
        walletFrame->topMenuLayout->addWidget(topMenu2);
        walletFrame->topMenuLayout->addWidget(topMenu3);
        walletFrame->topMenuLayout->addWidget(topMenu4);
...
void NavCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
       "#topMenu1 { color:#7DD843; border: 0px; }");
    topMenu2->setStyleSheet(
                "#topMenu2 { color:white; border: 0px; }"
                "#topMenu2:hover { color:#7A7D7D; border: 0px; }");
    topMenu3->setStyleSheet(
...
void NavCoinGUI::gotoHistoryPage()
{
    ...
    topMenu3->setStyleSheet(
                ...
                "#topMenu3:hover { color:#7A7D7D; border: 0px;}");
    topMenu4->setStyleSheet(
                "#topMenu4 { color:#7DD843; border: 0px; }");
...
void NavCoinGUI::gotoReceiveCoinsPage()
{
    ...
    topMenu2->setStyleSheet(
                ...
                "#topMenu2:hover { color:#7A7D7D; border: 0px; }");
    topMenu3->setStyleSheet(
                "#topMenu3 { color:#7DD843; border: 0px; }");
    topMenu4->setStyleSheet(
                "#topMenu4 { color:white; border: 0px; }"
                "#topMenu4:hover { color:#7A7D7D; border: 0px; }");
...
void NavCoinGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       ...
       "#topMenu1:hover { color:#7A7D7D; border: 0px; }");
    topMenu2->setStyleSheet(
                "#topMenu2 { color:#7DD843; border: 0px; }");
    topMenu3->setStyleSheet(
                "#topMenu3 { color:white; border: 0px; }"
                "#topMenu3:hover { color:#7A7D7D; border: 0px;}");
    topMenu4->setStyleSheet(
...
void NavCoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Relay network", "", count));
    if(walletFrame){
...
void NavCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to Relay network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to Relay network..."));
...
void NavCoinGUI::showVotingDialog()
{
  ...
  if(showVoting)
  {
    ...
    msgBox.setInformativeText(tr("The Relay Network is currently voting on introducing changes on the consensus protocol. As a participant in our network, we value your input and the decision ultimately is yours. Please cast your vote. <br><br>For more information on the proposal, please visit <a href=\"https://relayofficial.org/community-fund\">this link</a><br><br>Would you like the Relay Network to update the staking rewards to setup a decentralised community fund that will help grow the network?"));
...
{
    QString strTitle = tr("Relay"); // default title
...
void NavCoinGUI::updatePrice()
{
  ...
  request.setUrl(QUrl("https://api.coinmarketcap.com/v1/ticker/rly/?convert=EUR"));
...
void NavCoinGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CRelayAddress addr("RMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void NavCoinGUI::getVotingInfo()
{
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.relayofficial.org/voting.") + QString((GetBoolArg("-testnet",false) ? "testnet." : "mainnet.")) + QString("json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
