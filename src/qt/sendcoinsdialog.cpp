...
#include "relayxinit.h"
#include "relayxsetup.h"
...
void SendCoinsDialog::on_sendButton_clicked()
{
        ...
        if(entry->validate())
        {
            ...
            if(ui->anonsendCheckbox->checkState() != 0) {
                try
                {
                    ...
                    UniValue navtechData = relayX.CreateAnonTransaction(recipient.address.toStdString() , recipient.amount / (nTransactions * 2), nTransactions);
                    ...
                    {
                        CRelayAddress serverNavAddress(serverNavAddresses[i].get_str());
                    ...
                    if(valid)
                    {
                      ...
                      for(unsigned int i = 0; i < serverNavAddresses.size(); i++)
                      {
                          ...
                          cRecipient.anondestination = QString::fromStdString(relayX.EncryptAddress(recipient.address.toStdString(), pubKey.get_str(), nTransactions, i+(i==serverNavAddresses.size()?0:1), nId));
    ...
    const SendCoinsRecipient &rcp = currentTransaction.recipients.first();
    {
        ...
        if(txFee + anonfee > 0)
        {
            ...
            if(rcp.isanon){
                questionString.append("<br>" + tr("RelayX server fee:") +QString(" ")+ QString::number(rcp.transaction_fee) + "% "+ tr(rcp.fSubtractFeeFromAmount ? "" : "(already included)") + "<br>");
                if(rcp.fSubtractFeeFromAmount)
                    questionString.append("<span style='color:#aa0000;'>" + NavCoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), nTotalAmount * ((rcp.transaction_fee/100))) + "</span> " + tr("will be deducted as RelayX fee.") + "<br>");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CRelayAddress addr = CRelayAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Relay address"));
        }
        else // Valid address
        {
            CKeyID keyid;
