...
*Setup instructions for a Gitian build of Relay Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Relay
...
- [Building Relay Core](#building-relay-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-relay-host/README.md).
...
Clone the git repositories for relay and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/relay/relay-core
```
...
in the relay repository under 'Fetch and build inputs' to install sources which require
...
To build Relay Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the relay repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/relay/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/relaydev/relay-core
...
For example:
```bash
URL=https://github.com/relaydev/relay-core.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit relay-core=${COMMIT} --url relay-core=${URL} ../relay-core/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit relay-core=${COMMIT} --url relay-core=${URL} ../relay-core/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit relay-core=${COMMIT} --url relay-core=${URL} ../relay-core/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the relay git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../relay/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/relay-core/relay-detached-sigs.git

NAVPATH=/some/root/path/relay
SIGPATH=/some/root/path/relay-detached-sigs

./bin/gbuild --url relay=${NAVPATH},signature=${SIGPATH} ../relay/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/relay-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/relay-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/relay-osx-build.assert
```
...
[relay-core/gitian.sigs](https://github.com/relay-core/gitian.sigs/) repository, or if that's not possible create a pull
