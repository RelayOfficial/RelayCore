Relay Core version 1.0.1 is now available from:

  <https://relay.org/bin/relay-core-1.0.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/relay/relay/issues>

To receive security and update notifications, please subscribe to:

  <https://relaycore.org/en/list/announcements/join/>
...
updates will be released anymore. Without security updates, using a relay
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Relay Core
randomly crashing on Windows XP. It is [not clear](https://github.com/relay/relay/issues/7681#issuecomment-217439891)
what the source of these crashes is, but it is likely that upstream
libraries such as Qt are no longer being tested on XP.

We do not have time nor resources to provide support for an OS that is
end-of-life. From 1.0.1 on, Windows XP is no longer supported. Users are
...
- Adding `dbcache=100` in relay.conf
...
For example:
    ...
    $ src/relay-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Relay Core code base has
...
- `relay-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `relay-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
compatible peers.
[BIP 152](https://github.com/relay/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Relay Core.

[Pull request](https://github.com/relay/relay/pull/8035/files), [BIP 32](https://github.com/relay/bips/blob/master/bip-0032.mediawiki)
...
141](https://github.com/relay/bips/blob/master/bip-0141.mediawiki), [BIP
143](https://github.com/relay/bips/blob/master/bip-0143.mediawiki), [BIP
144](https://github.com/relay/bips/blob/master/bip-0144.mediawiki), and [BIP
145](https://github.com/relay/bips/blob/master/bip-0145.mediawiki) are
finished and included in this release.  However, BIP 141 does not yet specify
activation parameters on mainnet, and so this release does not support segwit
use on mainnet.  Testnet use is supported, and after BIP 141 is updated with
proposed parameters, a future release of Relay Core is expected that
implements those parameters for mainnet.

Furthermore, because segwit activation is not yet specified for mainnet,
version 1.0.1 will behave similarly as other pre-segwit releases even after a
future activation of BIP 141 on the network.  Upgrading from 1.0.1 will be
...
Witness)] (https://github.com/relay/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/relay/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the Relay-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/relay/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/relay/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP 
112](https://github.com/relay/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/relay/relay/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for relay-cli (laanwj)
...
- #7613 `3798e5d` Add autocomplete to relay-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add relay address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: relay-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from relayd (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the Relay-Core repository and website to the About Dialog (MarcoFalke)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for relayd procs (MarcoFalke)
...
- #8164 `0f24eaf` [Relay-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of relayd startup (laanwj)
...
- #8295 `f5660d3` Mining-related fixups for 1.0.1 (sdaftuar)
...
- #7975 `fc23fee` Update relay-core GitHub links (MarcoFalke)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/relay/).
