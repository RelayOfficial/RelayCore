...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "NavajoAnonSubChain/NavajoAnonSubChain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "NavajoAnonSubChain/NavajoAnonSubChain is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "NavajoAnonSubChain/NavajoAnonSubChain is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "NavajoAnonSubChain/NavajoAnonSubChain is downloading blocks...");
